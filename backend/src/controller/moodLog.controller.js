    ...
    try {
        ...
        const timeline = await MoodLog.aggregate([
            ...
            },
            {
                $group: {
                    _id: {
                        date: { $dateToString: { format: '%m/%d', date: '$createdAt', timezone: 'Asia/Kolkata' } },
                        emotion: '$emotion'
                    },
                    count: { $sum: 1 }
                }
        ...
        // Aaj ke last 5 logs — timeline list ke liye
        const today = new Date()
        today.setHours(0, 0, 0, 0)

        const recentLogs = await MoodLog
            .find({ userId: req.user.id, createdAt: { $gte: today } })
            .sort({ createdAt: -1 })
            .limit(5)
